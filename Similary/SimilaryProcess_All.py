...
def load_data(url):
    ...
    #保存id供后面使用
    id=power.id.iloc[0]
    #这里可以设置条件，让这些数据作为测试集，例如这里留下[149,150,151,152,153]
   ...
   # power.drop(columns='capacity')
    ...
    final_data.to_csv('./data/FinalData/similary_' + str(id) + '.csv',index=False,encoding='utf_8_sig')
    #final_data.to_csv('./data/FinalData/similary_' + str(id) + '.csv', index=False, encoding='utf_8_sig')
